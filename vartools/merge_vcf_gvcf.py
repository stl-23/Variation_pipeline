...
{bcftools} index {vcf}.sort.gz """.format(
        ...
        if len(files) < 1000:
            cmd = """{gatk4} --java-options "-Xmx{mem}g" CombineGVCFs -R {ref} {combine_gvcf} -0 {prefix}.combined.g.vcf
{gatk4} --java-options "-Xmx{mem}g" GenotypeGVCFs -R {ref} -V {prefix}.combined.g.vcf -G StandardAnnotation -newQual
-O {prefix}.combined.vcf""".format(gatk4=gakt4,mem=men,ref=ref,combine_gvcf=combine_gvcf, prefix=prefix)
...
{gatk4} --java-options "-Xmx{mem}g" GenotypeGVCFs -R {ref} -V gendb:{genomicsdb} -G StandardAnnotation -newQual
-O {prefix}.combined.vcf""".format(gatk4=gakt4,mem=mem,genomicsdb=genomicsdb,chr_list=chr_list,b_size=b_size,
